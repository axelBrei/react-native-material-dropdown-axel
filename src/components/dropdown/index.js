...
export default class Dropdown extends PureComponent {
  ...
  static propTypes = {
    ...
    itemTextProps: PropTypes.object,
  ...
  itemSize() {
    let { fontSize, itemPadding, itemTextProps } = this.props;

    return Math.ceil(fontSize * 1.5 + itemPadding * 2 * itemTextProps.numberOfLines);
  }
  ...
  renderItem({ item, index }) {
    ...
    return (
      <DropdownItem index={index} {...props}>
        <Text style={[styles.item, itemTextStyle, textStyle]} {...itemTextProps}>
          {title}
        </Text>
      </DropdownItem>
    );
